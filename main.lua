...
-- limit ball detecting range of AI
AI_VIEW_RANGE = VIRTUAL_WIDTH * 1 / 4
...
function love.update(dt)
    ...
    -- player 2 (AI)
    -- check if ball in view range, is moving toward AI's paddle, and ball's y axis is not between paddle's top and bottom
    -- if not, stop moving
    if ball.x >= AI_VIEW_RANGE and ball.dx > 0 and
        not (ball.y > player2.y and ball.y + ball.height < player2.y +
            player2.height) then
        -- Moving down, the more distance between the player's bottom and the ball's y-axis, the faster the paddle moving.
        if player2.y + player2.height < ball.y then
            player2.dy = math.min(PADDLE_SPEED, AI_SPEED +
                                      (ball.y - (player2.y + player2.height)))
        -- Moving up, similar to moving down
        elseif player2.y > ball.y + ball.height then
            player2.dy = math.max(-PADDLE_SPEED, -AI_SPEED -
